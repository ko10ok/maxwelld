...
from maxwelld.core.utils.compose_instance_cfg import get_compose_services
...
class ComposeInstance:
    ...
    async def run_env(self):
        compose_instance_files = await self.generate_config_files()
...
class ComposeInstanceManager:
    def __init__(self, project: str, compose_interface: type[ComposeShellInterface], except_containers: list[str],
                 compose_files_path: Path, in_docker_project_root: Path, host_project_root_directory: Path,
                 tmp_envs_path: Path):
        self.project = project
        self.compose_interface = compose_interface
        self.except_containers = except_containers
    ...
    def make(self, new_env_id: str, compose_files: str, config_template: Environment, ):
    ...
    def from_compose_instance_files(self, compose_files: str, config_template: Environment, ):
        return ComposeInstance(
            project=self.project,
            compose_interface=self.compose_interface,
            new_env_id=INFLIGHT,
    ...
    def down_env(self, env_id):
        ...
        down_result = compose_executor.dc_down(services)
