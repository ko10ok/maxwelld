...
services:
  s1:
    ...
    x-migration:
      - after_start: sh -c 'echo "migration after_start done" >> /tmp/migration.log'
      - after_all: sh -c 'echo "migration after_all done" >> /tmp/migration.log'
"""
        )
    ...
    async def then_it_should_apply_migration(self):
        ...
        assert self.migration_file_content == schema.bytes % (b'migration after_start done\n'
                                                              b'migration after_all done\n')
